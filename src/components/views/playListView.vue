  ...
  methods: {
    ...
    isNowPlayList(){
      if (this.isSearch == true) {
				return false;
			} else if (this.nowPlay.listName == this.nowPage && this.nowPlay.id == this.playList.id && this.nowPlay.nowPlayList.length != 0) {
				return true;
			}
			return false;
    },
      ...
      this.$nextTick(()=>{
        ...
        if(that.playList!=null && that.playList.id!=null && that.playList.id!=undefined){
          ...
          .then((response)=>{
            ...
            this.subTitle="合计"+response.playlist.songCount+"首歌"
          })
          .catch(()=>{
            // console.log(err);
